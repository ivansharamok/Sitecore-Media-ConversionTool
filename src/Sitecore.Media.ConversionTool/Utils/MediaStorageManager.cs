    ...
    public class MediaStorageManager
    {
        ...
        private static void DeleteFiles(IEnumerable<string> files)
        {
            foreach (string pathToFile in files)
            {
                ...
                {
                    Log.Error($"Error deleting file from path '{FileUtil.UnmapPath(pathToFile)}'", ex,
        ...
        {
            public static bool DeleteConvertedFiles => Configuration.Settings.GetBoolSetting("MediaStorageManager.DeleteConvertedFiles", false);

            public static bool EnableDetailedLogging => Configuration.Settings.GetBoolSetting("MediaStorageManager.EnableDetailedLogging", false);
        }
